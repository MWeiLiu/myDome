...
更新日志：
版本1.0.14 日期20161107 新增cselectorIOSCity IOS式城市选择器
版本1.0.13 日期20161105 新增cselectorDownCity下拉式城市选择器
版本1.0.12 日期20161027 新增mulaccept="image/*"属性 当支持H5的浏览器时 可以支持多图上传 不过需要后端支持
版本1.0.11 日期20160824 新增zjs.backcmddir备用命令目录 那么当服务器端接口没有开发完成报404的时候 可以转到前端接口 zjstips加代码 样式交由css处理
版本1.0.10 日期20160816 新增命令穿插其中 比如假数据的地址为/data/index.txt那么配置/data/{{cmd}}.txt 转换成服务器端模式后就变成了/api/1.0/{{cmd}}
版本1.0.09 日期20160728 新增默认的多行数据处理支持 在beforesubmit之后 默认会打包table[savepath]里面的数据 关键点为table[savepath]为字段名 table[savepathcopy]为数据同时送到的字段名 thead.hide为数据模板 tbody.item为每条数据 tbodyadd[target='']为字段名
版本1.0.08 日期20160618 修复bug 表格searchfull只支持一级 现已支持二级 伪静态的pageid id只有数字情况下才有效
版本1.0.07 日期20160320 增加keephtml接口 当form表单需要绑定真正的HTML内容时 使用该接口 否则默认过滤HTML 如果是老项目 就加一个zjs.keephtml=true; 就不再过滤HTML了
版本1.0.06 日期20160217 增加下拉刷新 只拉前端缓存的 缓存结束后 要手动点击下一页 这样可以防止无休止的下拉 看不到底部
版本1.0.05 日期20160210 如果是JSON方式提交数据 那么object对象里面的属性就在input上加上objectpath <input id="productname" objectpath="product" /> 这种结构就表示是product.productname关系
版本1.0.04 日期20160105 增加全文检索模块 关键字段名为searchfull
版本1.0.03 日期20151028 幻灯片默认为左右转场 增加fadein淡入淡出 showin手机端的淡入淡出
版本1.0.02 日期20151027 增加美图秀秀支持 可以对已上传的图片进行处理
版本1.0.01 日期20151023 增加beforerenderrow回调 在每一行数据渲染之前调用
*/
//声明zjs命名空间
if (!window.zjs) { zjs = {}; };
if (!window.zjss) { zjss = {} };
    ...
    if (zjs.lhgtips) {//如果有lhg的 就表示是PC端 按PC端提示
        if (succ != undefined) {//如果没有传 就显示是loading
            if (zjs.checkrep(succ)) {//如果大于0 表示失败 
                ...
                zjs.alert("<img class='fl' src='" + zjs.rootdir + "js/qq/9.gif'/>&nbsp;" + message);
            } else {//小于等于0表示成功
                if (e && e.isalert) {
                    zjs.lhgtips("", 0.1, "loading.gif");
                    alert(message);
                } else {
                    zjs.lhgtips(message, (time || 2), "s.png");
                }
            }
        } else {
...
/*日期格式化*/
zjs.dateformat = function (value, dateformat, htmlbox, colname) {
    if (value && dateformat) {
        ...
        if (reg.test(value) && value.toString().length < 20) {
            var result = value.match(reg);//将字符串以正则表达规则拆成字符数组
            ...
            if (dateformat == "diff") {//如果是朋友圈 几分钟前 几小时前 之类的格式 就调这个函数
        ...
        if ((value + "").indexOf("<") > -1) {//如果值里面有< 这种HTML的起始关键元素的话 就把整个值HTML编码一下
            if (htmlbox) {//如果传的有htmlbox 说明有非强制过滤字段 
                if (htmlbox.indexOf(colname) > -1) {
                    ...
                    return zjs.htmlEncode(value);
                }
            } else {//如果没传htmlbox 而元素中有HTML 就直接过滤
                return zjs.htmlEncode(value);
            }
        } else {//如果元素里没有HTML标签 就不用管了
            return value;
        }
    } else {
        return value;
...
/*日期格式化*/
zjs.dateformati = function (value, dateformat) {
    if (value && dateformat) {
        var reg = /(\d{4})\S(\d{1,2})\S(\d{1,2})[\S\s](\d{1,2}):(\d{1,2}):(\d{1,2})/;
        var regdate = /(\d{4})\S(\d{1,2})\S(\d{1,2})/;
        if (reg.test(value) && value.toString().length < 20) {
...
zjs.renderRow = function (datai, render, dateformat, htmlbox) {
    ...
    for (var attr in datai) {
        if (datai[attr] == null)
            datai[attr] = "";
        row = row.replace(new RegExp("{{" + attr + "}}", 'g'), zjs.dateformat(datai[attr], dateformat, htmlbox, attr));
    }
    return row;
};
/*数组渲染*/
...
zjs.openurl = function ($t, url, callback) {
    var owidth = $t.attr("owidth") || 960;//宽
    ...
    return zjs.dialog({//调用lhgdialog
        ...
        close: function () {
            try {
                if ($t.attr("callback")) {//如果被点击的对象有回调 也调
    ...
    zjs.ctable = {
        ...
        _init: function (config) {//内部函数 初始化
            ...
            if (!that.render) {
                ...
                $body.empty().html("<img src='" + zjs.rootdir + "img/loading.gif' class='loading'/>").show();//先清空
                ...
                if ($batchSelection.length > 0) {
                    $batchSelection.click(function () {
                        if ($(this).attr("checked")) {//如果选中
                            $body.find(".selection").attr("checked", $(this).attr("checked"));
                            $batchSelection.attr("checked", $(this).attr("checked"));
                        } else {//如果没选中
                            $body.find(".selection").removeAttr("checked");
                            $batchSelection.removeAttr("checked");
                        }
                    });
                };
                //this._search(null, true);
                var $batchdelete = $id.find("a[batchdelete]");
                            ...
                            if (ceditorDialogC.content.document.getElementById("isclose")) {//如果是确认关闭的 而不是直接关闭
                                if (ceditorDialogC.content.document.getElementById("isclose").value) {//如果是确认关闭的 而不是直接关闭
                                    ...
                                    if (that.lockrender) {//锁表头模式 要更新左侧和右侧
                                        ...
                                        $oldlockrow.after(zjs.renderRow(datai, that.lockrender));
                                        $oldlockrow.remove();
                                    } else { //普通模式 按以前的处理
                                        $t.parents(".uls").after(zjs.renderRow(datai, that.render));
            ...
            if ($id.attr("nomsg") == "true") {//不提示 
                ...
                if ($id.find("thead").length > 0) {
                    if ($id.find("thead .nomsg").length == 0) {
                        var $nomsg = $("<tr class='nomsg'><td colspan='100'>未搜索到相关结果</td></tr>");
        ...
        dosearch: function () {//执行搜索 如果数据都在前端 就在前端搜索 否则 就发到服务端处理
            ...
            that.$body.empty().html("<img src='" + zjs.rootdir + "img/loading.gif' class='loading'/>");//先清空Tbody 并变菊花
            ...
            if (that.maxTotle == that.Datas.length && that.Datas.length > 0 && $id.attr("onlyserver") != "true") {
                var id = $id.attr("id");
        ...
        _search: function (searchpara, clearCache) {//搜索服务器端
            ...
            if ($id.attr("cmd-select")) {//页面加载后 如果配置有获取数据URL 就向URL发起请求 
                ...
                if (clearCache)
                    that.$body.empty().html("<img src='" + zjs.rootdir + "img/loading.gif' class='loading'/>");//先清空
        ...
        _render: function (clearbody) {//行渲染模式
                    ...
                    if ($id.attr("lockall")) {
                        ...
                        if ($id.attr("scrollToRefresh")) {
                            if ($id.hasClass("ctable"))
                                $body.append("<tr><td colspan='100'><div class='pagerpostion'><a name='pager" + that.uipageindex + "'>" + that.uipageindex + "</a></div></td></tr>");
                            else
                                $body.append("<div class='c12'><div class='pagerpostion'><a name='pager" + that.uipageindex + "'>" + that.uipageindex + "</a></div></div>");
                        }
            ...
            if ($pageNumbers.length > 0) {
                ...
                if ($id.attr("scrollToRefresh")) {//如果是下拉刷新 就只能加载目前已有的页码 点击为锚点
                    ...
                    if (pagestart > 10) {
                        $pageNumbers.append("<a href='#pager1' class='btn'>1</a>");
                    }
                    for (var i = pagestart; i <= pageend && i <= TotalPages; i++) {
                        $pageNumbers.append("<a href='#pager" + i + "' class='btn " + (i == PageIndex ? "hover" : "") + "'>" + i + "</a>");
                    }
                }
                else {
                    ...
                    for (var i = pagestart; i <= pageend && i <= TotalPages; i++) {
                        $pageNumbers.append("<a href='javascript:void(0)' class='btn " + (i == PageIndex ? "hover" : "") + "'>" + i + "</a>");
                    }
        ...
        render: function () {//渲染表单
            ...
            if (that.template) {
                ...
                for (var di in datai) {
                    ...
                    //如果属性本身就是object对象 就转一下字符串 再放进去
                    var dvalue = typeof datai[di] == "object" ? $.toJSON(datai[di]) : datai[di];
                    var dtext = zjs.dateformat(dvalue, $id.attr("dateformat"));//通过日期格式化一下

                    var $did = $id.find("#" + di);
                    if ($id.find("span#" + di).length > 0) {//如果是个span 就展示
                        $did.text(dtext);
                    } else if ($id.find("div#" + di).length > 0) {//如果是个span 就展示
                        $did.text(dtext);
                    } else {
                        if (datai[di] == null) {//如果是null 就置空
                            $did.val("").trigger("change");
                        } else {
                            if ($did.hasClass("ceditor") || $did.hasClass("ceditormin") || $did.hasClass("htmlbox")) {
                                $did.val(dvalue).trigger("change");
                            } else {
                                $did.val(zjs.htmlDecode(dtext)).trigger("change");
                            }
                        }
                    }
                }
        ...
        _save: function () {
            ...
            if (!that.validator() || $id.find("#submit").hasClass("loading")) {
                ...
                $id.find("table[savepath]").each(function (i, v) {//20160726 新增可以在表单中打包多行数据的默认支持
                    ...
                    if ($(v).attr("savepathcopy")) {
                        var savepathcopy = $(v).attr("savepathcopy").split(',');
                        for (var scindex in savepathcopy) {
                            $id.find("#" + savepathcopy[scindex]).val($.toJSON(items));
                        }
                    }
                });
...
zjs.cselector_ulclick = function ($input, $ul, $id, nothide, isunbind) {
    ...
    $ul.find("a.readonly").click(function (e) {
        e.stopPropagation();
    });
};
//当文本框的值发生变更时执行的事件
...
if (!window.zjs.cselector) {
    zjs.cselector = {
                ...
                switch (that.cselectorType) {
                    ...
                    case "cselectorTree"://下拉树  
                        ...
                        if ($id.attr("jsontree")) {//如果数据结构为json数据
                            ...
                            that.$ul.append(zjs.tree(uldata, zjs.treeroot,
                                $id.attr("template"),
                                ($id.attr("idkey") || "classifyid"),
                                ($id.attr("pidkey") || "parentid")));//追加HTML
                        }
            ...
            switch (this.cselectorType) {
                ...
                default://普通下拉列表框
                    ...
                    if ($id.attr("isinput")) {
                        ...
                        $isinput.blur(function () {//当文本框失去焦点时 如果文本框的值 不是列表中的值 就重置文本框
        ...
        isinputchange: function () {
            ...
            var $ul = this.$ul;
            var $isinput = this.$isinput;
            clearTimeout(that.isinputchangetimer);//延迟500毫秒 防止连续输入
            that.isinputchangetimer = setTimeout(function () {
    ...
    zjs.cselectorImageUpload = {
        ...
        _event: function () {
            ...
            $ifrm.bind("load", function () {
                ...
                clearInterval(timer);
                if (window.imageMessage) {
                    zjs.lhgtips(window.imageMessage, 1.5);
                    window.imageMessage = null;
                    $divu.html("");
                } else {
                    if ($id.attr("lock") && window.imageupload) {
                        zjs.lhgtips("上传完成", 0.1, "loading.gif", true);
                    }
                }
...
zjs.documentReady = function () {
    ...
    $("i[dateformat]").each(function (i, v) {//日期格式化
        $(v).html(zjs.dateformati($(v).text(), $(v).attr("dateformat"))).removeAttr("dateformat");
    });
...
$(function () {//dom加载完成后 初始化表单 表格等
    ...
    $(".tbodyadd").live("click", function () {
        ...
        if ($t.attr("maxlength")) {
            if ($table.children("tbody").length >= $t.attr("maxlength")) {
