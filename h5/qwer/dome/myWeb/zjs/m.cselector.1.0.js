...
更新日志：
版本1.0.6 日期20160728 新增默认的多行数据处理支持 在beforesubmit之后 默认会打包table[savepath]里面的数据 
        关键点为table[savepath]为字段名 table[savepathcopy]为数据同时送到的字段名 thead.hide为数据模板 tbody.item为每条数据 tbodyadd[target='']为字段名
版本1.0.5 日期20160618 修复bug 表格searchfull只支持一级 现已支持二级 伪静态的pageid id只有数字情况下才有效
版本1.0.4 日期2015.10.27 增加美图秀秀支持
版本1.0.3 日期2015.4.25 增加表单提交时的 表单编号+submitpara 提交参数勾子 就可以实现 在提交时附加其他参数 而不用隐藏的input
版本1.0.2 日期2015.4.25 增加表格中第一页 和下一组按钮 可以直接跳转第一页或者下一组的第一页 下一组是靠cachePage变量定义 目前为10 缓存10页
版本1.0.1 日期2015.4.23 增加可复选树型选择器
*/
//声明zjs命名空间
if (!window.zjs) { zjs = {}; };
if (!window.zjss) { zjss = {} };
...
zjs.tips = function (message, succ, time,e) {
    if (zjs.lhgtips) {//如果有lhg的 就表示是PC端 按PC端提示
        ...
        $("div.zjstips").remove();
        var $msg = $("<div class='zjstips'>" + message + "</div>");
        if (succ != undefined) {//如果没有传 就显示是loading
            if (zjs.checkrep(succ)) {//如果大于0 表示失败
                $msg.append("<img src='" + zjs.rootdir + "img/e.png' align='left' />");
            }
            else {//否则表示成功
                $msg.append("<img src='" + zjs.rootdir + "img/s.png' align='left' />");
            }
        } else {
                    ...
                    callback: function (data) {
                        ...
                        if (data) {
                            if (clearCache) {//如果是搜索状态 就清空缓存中的所有数据
                                that.Datas = data.Datas || data.datas || data.Data;
                                that.pageindex = 1;//重置服务器端当前页码
                                that.uipageindex = 1;//重置前端当前页面
                                //that.maxTotle = 0; //2015-11-25 由于重置了maxTotle所以前端查询失效了 所以注释
                                that.searchData = [];
                            } else {//如果是下拉刷新的下一页 就把数据追加到现有数据集中
                                that.Datas = that.Datas.concat(data.Datas || data.datas || data.Data);//数组对数组的追加是用concat
                            }
                        ...
                        var spara = {//如果没有缓存 就发起命令读数据
                            ...
                            callback: function (data) {
                                if ($id.attr("parentKey")) {//放入缓存
                                    part[$id.attr("parentKey")] = data.Datas || data.Data || data;
                                ...
                                uldata = data.Datas || data.Data || data;//存放数据 
                            }
                        };
                        if ($id.attr("contentType")) {
                            spara.contentType = $id.attr("contentType");
        ...
        _event: function () {
            ...
            function filechange() {
                ...
                if (typeof FormData == "function") {//FormData是HTML5新增的函数 如果支持这个函数 就使用HTML5上传方式 
                    var fd = new FormData();
                    fd.append("fileToUpload", $file[0].files[0]);//设置要上传的文件
...
if (!window.zjs.cfocus) {
    zjs.cfocus = {
        ...
        _init: function (config) {
            ...
            if ($id.attr("thumbnail")) {//fixed表示固定高度
                ...
                $items.each(function (i, v) {
                    $page.append("<a><i class='iconfont'>&#xe656;</i></a>");//追加页码
                });
            }
            ...
            if ($items.length > 1) {
                ...
                function goItem(isback, isfirst) {
                    ...
                    $pageItems.eq(i).addClass("hover").siblings().removeClass("hover");//显示对应页码
